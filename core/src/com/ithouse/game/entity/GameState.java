...
import com.ithouse.game.level.Level;
import com.ithouse.game.level.Map;
...
public class GameState {
    ...
    private void createMap() {
        grid = new Tile.TileType[GRIDX][GRIDY];
        for (int i = 0; i < GRIDX; i++) {
            for (int j = 0; j < GRIDY; j++) {
                grid[i][j] = Tile.TileType.Used;
            }
        }
    }
    ...
    public void checkForEnemySpawn(float delta) {
        ...
        if (spawnDelay >= .5 && spawnedEnemies < enemiesToBeSpawned.size()) {
            ...
            Enemy.EnemyType type = Enemy.interpretType(enemiesToBeSpawned.get(spawnedEnemies));
    ...
    public boolean isTowerPlaceable(Point point) {
        try {
            return point.x > 0 && point.y > 0 && grid[point.x / 40][point.y / 40] != Tile.TileType.Dirt;
        } catch (Exception e) {
