...
import com.ithouse.game.screen.GameUserInterface;
import com.ithouse.game.td.Config;
import com.ithouse.game.td.GDSprite;
import com.ithouse.game.td.helper.MathHelper;
...
public class GameInputProcessor extends GDInputProcessor {
    ...
    public GameInputProcessor(UnrealDefence unrealDefence) {
        super(unrealDefence);
    ...
    public boolean keyDown(int keycode) {
        if (keycode == 27) {
            ...
            unrealDefence.switchScreen(unrealDefence.getPauseScreen());
        }
        return false;
    }
    ...
    private void selectTowerToBuild(int x, int y, int pointer, int button) {
        ...
        for (int type = 0; type < availableTowers.size(); type++) {
            ...
            if (sprite.contains(x, y)) {
                TowerFactory.TowerType towerType = TowerFactory.interpretType(type);
...
//				Point spritePoint = sprite.getPosition();
                ...
                TowerFactory.TowerType towerType = TowerFactory.interpretType(i);
                towerToBuild = TowerFactory.createTower(towerType);
