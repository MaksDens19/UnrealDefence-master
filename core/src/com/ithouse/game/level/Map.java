package com.ithouse.game.level;
...
public class Map {
    ...
    public static HashMap<Integer, Tile.TileType[][]> maps = new HashMap<Integer, Tile.TileType[][]>();

    public static Tile.TileType[][] generateMap(int i) {
        Tile.TileType[][] map = maps.get(i);
        if (maps.get(i) == null) {
            switch (i) {
                case 1:
    ...
    private static Tile.TileType[][] convertStringToGrid(String[] grid) {
        int x = grid[0].length(), y = grid.length;

        Tile.TileType[][] map = new Tile.TileType[x][y];
